...
# validar numeros en query params 
@app.get("/validar-numeros/{item}")
async def validar_numeros(
        item: int = Path(..., ge=0, le=100), # ge = greater than, le = less than
        q: Optional[str] = Query(None, min_length=3, max_length=50, regex="^pepe")
        ):
    results = {"item": item, "q": q}
    return results
